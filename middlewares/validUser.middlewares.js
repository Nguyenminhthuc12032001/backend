...
const validUser = [
    body('name').trim().notEmpty().withMessage('Username cannot be empty'),
    body('email').trim().notEmpty().withMessage('Email cannot be empty'),
    body('phone_number').trim().notEmpty().withMessage('Phone number cannot be empty'),
...
const validUserUpdate = [
    ...
    body('phone_number').optional().trim().notEmpty().withMessage('Phone number cannot be empty'),
    body('password_hash').optional().trim().notEmpty().withMessage('Password cannot be empty'),
    body('confirmPassword').optional().trim().notEmpty().withMessage('Confirm Password cannot be empty')
        .custom((value, {req}) => {
            if (value !== req.body.password) {
                throw new Error('Confirm Password must match Password');
            }
